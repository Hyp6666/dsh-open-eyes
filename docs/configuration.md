...
The browser endpoints are fixed at `/vision-bridge/v1/web-image-route` and `/vision-bridge/v1/web-drafts`, same-origin POST-only, and not user-configurable. They accept no arbitrary endpoint or header. The routing endpoint is read-only. Draft per-image/count/aggregate admission uses `maxImageBytes`, `maxImages`, and the fixed 64 MiB aggregate limit. Installation or package upgrade requires a Web process restart because DSH client-package metadata is process-cached; edits to this row's `config` remain subject to DSH's normal hot reload.
