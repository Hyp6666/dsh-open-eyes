...
The Web endpoints accept only same-origin JSON POSTs for a current SessionStore session. Cross-site requests, absent/mismatched Origin, other methods, other content types, invalid base64, MIME spoofing, too many images, and byte overflows fail before persistence. The capability endpoint is read-only and accepts only bounded session/provider/model identifiers. The history-image endpoint accepts only a complete plugin token, extracts its session identity, and requires that exact token to occur on its own line in a direct user message of that live session before `readImage`; an attachment id by itself is insufficient. Draft request buffering, decoded bytes, image count, and aggregate bytes are bounded. Every image is magic-byte checked and decoded through `validateImage`; only after the whole batch passes is each item committed with `saveImage`.
...
The client registers the `conversation.chat.node` slot's `user` and `steering` cells below the stock conversation renderers. For rows whose durable text contains bridge attachment links it derives a view-only content array (the exact user text plus native image-attachment blocks decoded from the link tokens) and hands it to the stock renderer; every other row is rendered by the stock components with untouched props, and a missing stock renderer renders nothing rather than a degraded row. The projection never mutates the durable node or writes to the attachment store. Bridge thumbnails use the fixed session/log-authorized endpoint described above because DSH's native attachment RPC intentionally rejects references that are not native image blocks. The browser retains only a bounded LRU of object URLs (128 images and 64 MiB) and revokes them on eviction or plugin unload. The durable user turn remains exactly the text the model consumed, so transcript auditing is unaffected by the display upgrade.
